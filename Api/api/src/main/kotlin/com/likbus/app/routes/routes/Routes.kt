...
fun Route.routes() {
    get("/stops/{stopID}/buses/{busID}/schedule") {
        val stopID = runCatching {  UUID.fromString(call.parameters["stopID"]) }.getOrNull() ?: return@get call.respond(HttpStatusCode.UnprocessableEntity)
        val busID = runCatching {  UUID.fromString(call.parameters["busID"]) }.getOrNull() ?: return@get call.respond(HttpStatusCode.UnprocessableEntity)
        ...
        val routeStopSchedule = RouteStopSchedule(
            ...
            times = routeStopRaw.map { it.time }
        )
